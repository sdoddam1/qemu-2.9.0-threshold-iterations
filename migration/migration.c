...
//#ifdef HYPERFRESH
                //printf("Iterations %d page count %d s->bytes_xfer %lu s->xfer_limit %lu\n", iterations, page_count, s->bytes_xfer, s->xfer_limit);
                page_count = 0;
#ifdef HYPERFRESH
        	iterations++;
#endif
...
//Hyperfresh
	    //printf("transferred bytes = %lu time spent(ms) = %lums bandwidth = %f\n", transferred_bytes, time_spent, bandwidth);
...
//	printf("dirty bytes rate %" PRId64 "dirty pages rate %" PRId64 "\n", s->dirty_bytes_rate, s->dirty_bytes_rate);
        ...
        if (qemu_file_rate_limit(s->to_dst_file)) {
	    //printf("sleeping for %lu\n",(initial_time + BUFFER_DELAY - current_time)*1000);
